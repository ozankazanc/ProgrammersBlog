                ...
                action: function (e, dt, node, config) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            dataTable.clear(); // ilk önce datatablei temizleyip sonra değerleri basıyoruz.
                            console.log(userListDto);
                            if (userListDto.ResultStatus == 0) {
                                $.each(userListDto.User.$values,
                                    function (index, user) {
                                       var rowNode= dataTable.row.add([
                                           ...
                                           `<img src="/img/${user.Picture}" alt="${user.UserName}" class="my-image-table"/>`,
                                            `<div class="btn-group" role="group">
                                                <button class="btn btn-primary btn-sm btn-update" data-id="${user.Id}"><span class="fas fa-edit"></span> Düzenle</button>
                                                <button class="btn btn-danger btn-sm btn-delete" data-id="${user.Id}"><span class="fas fa-minus-circle"></span> Sil</button>
                                            </div>`
                                       ]).node(); //burada .draw(); kullanmadık çünkü tek bir değer ekleme yapmıyoruz.
                                        rowNode.setAttribute("name", `${user.Id}`);
                                    });
                                dataTable.draw(); // burada draw kullanıyoruz.
                                $(".spinner-border").hide();
                ...
                success: function (data) {
                    ...
                    if (isValid) {
                        ...
                        const rowNode = dataTable.row.add([
                            ...
                            `<div class="btn-group" role="group">
                                  <button class="btn btn-primary btn-sm btn-update" data-id="${userAddAjaxModel.UserDto.User.Id}"><span class="fas fa-edit"></span> Düzenle</button>
                        ...
                        rowNode.setAttribute("name", `${userAddAjaxModel.UserDto.User.Id}`);
...
/* Yeni User ekleme alanının bittiği yer */
    /*User deleted alanı*/
    $(document).on("click", ".btn-delete", function (event) {
        event.preventDefault();
        const id = $(this).data("id");
        const tableRow = $(`[name="${id}"]`);
        const userName = tableRow.find('td:eq(1)').text(); // ikinci td de ki veriyi çekecektir.
        Swal.fire({
    ...
    /*User update modal doldurma alanı*/
    $(function myfunction() {
        ...
        $.ajax({
            ...
            success: (function (data) {
                ...
                if (userUpdateAjaxModel.UserDto !== null) {
                    const rowid = userUpdateAjaxModel.UserDto.User.Id;
                    const tableRow = $(`[name=${rowid}]`); //kullanıcı bilgilerinin bulunduğu table row
                }
                ...
                if (isValid) {
                    ...
                    tableRow.attr("name", `${rowid}`);
                    dataTable.row(tableRow).invalidate(); //güncelleme işlemi yapıyor.
